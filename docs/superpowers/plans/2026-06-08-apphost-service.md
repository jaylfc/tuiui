# Apphost Service — Implementation Plan
...
install/uninstall/status. `main.rs` gains a `service` subcommand. `apphost::server::run` becomes
idempotent. Runtime platform dispatch via `cfg!(target_os=…)` (whole module compiles everywhere).

**Tech Stack:** Rust 2021, `std::process::Command` (launchctl/systemctl), `dirs`, `libc` (getuid),
existing `crate::protocol::apphost_socket_path`.
...
## Task 1: Make the apphost idempotent

**Files:** Modify `src/apphost/server.rs`

- [ ] **Step 1:** At the very start of `pub fn run()`, before binding, probe for an existing apphost:
```rust
pub fn run() -> std::io::Result<()> {
    ...
    // ... rest unchanged ...
```
(Keep the rest of `run()` as-is. `apphost_socket_path` is already imported.)

- [ ] **Step 2:** Build + the existing apphost tests still pass.
```bash
export PATH="$HOME/.cargo/bin:$PATH"
cargo test apphost:: 2>&1 | tail -15
cargo build 2>&1 | tail -5
git add src/apphost/server.rs
git commit --no-verify -m "apphost: exit cleanly if another instance is already listening (idempotent)"
```

---

## Task 2: `src/service.rs` — generators + install/uninstall/status

**Files:** Create `src/service.rs`; modify `src/lib.rs` (`pub mod service;`)
...
pub enum Backend { Launchd, Systemd, Profile, Unsupported }
...
pub fn launchd_plist(label: &str, exe: &str, env: &[(String, String)]) -> String {
    ...
    for (k, v) in env {
        env_xml.push_str(&format!("    <key>{}</key><string>{}</string>\n", k, xml_escape(v)));
    }
    format!(
...
- [ ] **Step 2:** Detection + paths + the effectful install/uninstall/status:
```rust
...
fn has_user_systemd() -> bool {
    match Command::new("systemctl").args(["--user", "show-environment"]).output() {
...
pub fn backend() -> Backend {
    if cfg!(target_os = "macos") {
        ...
        if has_user_systemd() { Backend::Systemd } else { Backend::Profile }
    } else {
        Backend::Unsupported
    }
}
...
fn launchd_plist_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join("Library/LaunchAgents").join(format!("{LAUNCHD_LABEL}.plist")))
}
...
pub fn install() -> io::Result<()> {
    ...
    match backend() {
        Backend::Launchd => {
            ...
            let _ = Command::new("launchctl").args(["bootout", &domain, &ps]).status();
            let st = Command::new("launchctl").args(["bootstrap", &domain, &ps]).status()?;
            if !st.success() {
        ...
        Backend::Profile => {
            ...
            let sock = crate::protocol::apphost_socket_path().to_string_lossy().into_owned();
...
pub fn status() -> io::Result<()> {
    ...
    match backend() {
        Backend::Launchd => if let Some(p) = launchd_plist_path() {
            println!("  installed: {}", p.exists());
            println!("  plist:    {}", p.display());
        },
        Backend::Systemd => {
            if let Some(p) = systemd_unit_path() { println!("  unit:     {} (installed: {})", p.display(), p.exists()); }
            let _ = Command::new("systemctl").args(["--user", "--no-pager", "status", SYSTEMD_UNIT]).status();
        }
        Backend::Profile => if let Some(p) = profile_path() {
            let has = std::fs::read_to_string(&p).map(|t| t.contains(PROFILE_START)).unwrap_or(false);
...
fn strip_profile_block(text: &str) -> String {
    ...
    for line in text.lines() {
        if line.trim() == PROFILE_START { skipping = true; continue; }
        if line.trim() == PROFILE_END { skipping = false; continue; }
...
- [ ] **Step 3:** Register module in `src/lib.rs`: `pub mod service;`.

- [ ] **Step 4:** Unit tests (pure generators + strip):
```rust
#[cfg(test)]
mod tests {
    use super::*;
    fn env() -> Vec<(String, String)> { vec![("PATH".into(), "/usr/bin:/bin".into()), ("HOME".into(), "/home/x".into())] }
...
- [ ] **Step 5:** Test + clippy + (macOS) validate the plist:
```bash
export PATH="$HOME/.cargo/bin:$PATH"
cargo test service:: 2>&1 | tail -15
cargo build 2>&1 | tail -5
...
git commit --no-verify -m "service: per-user apphost service generators + install/uninstall/status (launchd/systemd/profile)"
```
...
```
Update the `Some(other)` unknown-command hint and the top-of-file doc comment to list `service`.

- [ ] **Step 2:** Build + full suite + clippy + commit:
```bash
export PATH="$HOME/.cargo/bin:$PATH"
cargo build 2>&1 | tail -5
...
## Task 4: Validate + docs (controller)
...
- **Compiles everywhere:** the module is runtime-dispatched (`cfg!`), not `#[cfg]`-gated, so all
...
- **Env baking:** `service_env()` captures PATH/HOME/SHELL/LANG at install time so the service apphost
  can launch apps; documented that re-installing refreshes them.
