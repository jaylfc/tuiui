...
2. **Process / PTY host.** One per running app: spawns the app in a [`portable-pty`](https://docs.rs/portable-pty) PTY, parses output with a full [`alacritty_terminal`](https://docs.rs/alacritty_terminal) emulator into a screen grid, exposes that grid as a compositor layer, forwards keyboard/mouse into the app. *(The spec originally chose `vt100`; it proved too minimal to render demanding TUIs like `btop` without artifacts, so the host was swapped to `alacritty_terminal` — Alacritty's complete, embeddable VT core — during Slice 1.)*
