...
- **AI assistant** — the **✦** menubar button (next to the view toggle) opens a persistent chat panel running your coding agent. Recommended: **Claude Code** (reads the injected `CLAUDE.md` briefing natively) or **[opencode](https://github.com/sst/opencode)** (open source, many providers incl. local models, reads `AGENTS.md`); **[smallcode](https://github.com/Doorman11991/smallcode)** is supported for fully-local small models (briefing lands in its `knowledge/` dir). The agent is briefed that it lives inside tuiui: it can read `~/tuiui-debug.log` to diagnose problems, drive the desktop via the control CLI (`tuiui launch <app>`, `tuiui tile`, `tuiui theme <name>`, `tuiui msg '<json>'`), fix app-install issues, and even clone the tuiui repo to fix bugs and open PRs. The panel is a normal apphost window, so the conversation **survives detach and reloads**. Pin a specific agent with `assistant_command` / `assistant_args` in config.toml; otherwise the first of claude/opencode/smallcode/kilo/hermes/openclaw on `$PATH` is used.
...
Exit/Restart/Shutdown live in the top-right **host-name menu** (`▾`): **Exit** detaches, **Restart** reloads the UI keeping apps alive, **Shutdown** stops everything. **Systems** cascades the machine switcher: click a saved machine to ssh into its tuiui session (its `✕` forgets it), or **+ Add Remote…** to set up a new one (Tab/↑↓ between fields, `←`/`→` picks the theme, Enter connects). Troubleshooting a switch: every step logs to `~/tuiui-debug.log` (open it in-app via launcher → tuiui → **Logs**, press `c` to copy it); `TUIUI_DEBUG=1` additionally prints the exact ssh/setup script before it runs.
